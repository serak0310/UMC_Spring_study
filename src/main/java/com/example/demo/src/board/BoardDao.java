...
import com.example.demo.src.board.model.PatchBoardOwnerReq;
...
public class BoardDao {
    ...
    */
    ...
    // 게시판 소유자 변경
    public int modifyBoardOwner(PatchBoardOwnerReq patchBoardOwnerReq) {
        String modifyBoardOwnerQuery = "update Board set userIdx= ? where boardIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyBoardOwnerParams = new Object[]{patchBoardOwnerReq.getNewOwnerIdx(), patchBoardOwnerReq.getBoardIdx()}; // 주입될 값들(nickname, userIdx) 순
        return this.jdbcTemplate.update(modifyBoardOwnerQuery, modifyBoardOwnerParams); // 대응시켜 매핑시켜 쿼리 요청(생성했으면 1, 실패했으면 0)
    }
